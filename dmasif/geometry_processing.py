...
def subsample(x, batch=None, batch_atom=None, scale=1.0):
    ...
    if batch is None:  # Single protein case:
        ...
        bs = torch.zeros_like(batch_atom[:C])
        bs.scatter_add_(0, labels[:, None], batch_atom)
        return (points[:, :-1] / points[:, -1:]).contiguous(), (bs / points[:, -1:]).contiguous().round().long().squeeze()
        ...
        points, batches, batches2 = [], [], []
        for b in range(batch_size):
            idx = batch == b
            if idx.sum() == 0:
                continue
            p, bs_atom = subsample(x[idx], batch_atom=batch_atom[idx].unsqueeze(-1), scale=scale)
            points.append(p)
            batches.append(b * torch.ones_like(batch[: len(p)]))
            batches2.append(bs_atom)
    return torch.cat(points, dim=0), torch.cat(batches, dim=0), torch.cat(batches2, dim=0)
...
def soft_distances(x, y, batch_x, batch_y, smoothness=0.01, atomtypes=None):
    ...
    if atomtypes is not None:
        ...
        # ).view(-1)
        mean_smoothness = (-D_ij.sqrt()).exp().sum(0)   # (M)
        mean_smoothness_j = LazyTensor(mean_smoothness[None, :, :]) # (1, M)
        mean_smoothness = (
...
def atoms_to_points_normals(
    ...
    batch2 = torch.arange(N).long().to(atoms.device)
    batch_z2 = batch2[:, None].repeat(1, B).view(N * B)
    ...
    with torch.enable_grad():
        ...
        batch_z2 = batch_z2[mask].contiguous().detach()

        # e) Subsample the point cloud:
        points, batch_points, batch_z2 = subsample(z, batch_z, batch_z2, scale=resolution)
        # batch_points = batch[batch_z2]
    ...
    return points.detach(), normals.detach(), batch_points.detach(), batch_z2.detach()
