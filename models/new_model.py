...
class PositionEncoder:
    def __init__(self, multires, include_input=True, input_dims=3):
...
class TensorProductScoreModel(torch.nn.Module):
    def __init__(self, t_to_sigma, device, timestep_emb_func, in_lig_edge_features=4, sigma_embed_dim=32, sh_lmax=2,
                 ...
                 confidence_dropout=0, confidence_no_batchnorm=False, num_confidence_outputs=1, use_pos=False):
        ...
        self.use_pos = False
        if self.use_pos:
            self.Pos_embeder = PositionEncoder(multires=7)
            self.pos_embed_dim = self.Pos_embeder.out_dim
        ##############
        ...
        if self.confidence_mode:
            ...
            input_dim = 1 + sigma_embed_dim
            if self.use_pos:
                input_dim += self.pos_embed_dim
            self.tr_final_layer = nn.Sequential(nn.Linear(input_dim, ns),nn.Dropout(dropout), nn.ReLU(), nn.Linear(ns, 1))
    ...
    def forward(self, data):
        ...
        if self.use_pos:
            pos_embed = self.Pos_embeder.embed(data['ligand'].pos.mean(dim=0, keepdim=True)).to(data['ligand'].x.device)
            input_data = torch.cat([tr_norm, data.graph_sigma_emb, pos_embed], dim=1)
        else:
            input_data = torch.cat([tr_norm, data.graph_sigma_emb], dim=1)
        tr_pred = tr_pred / tr_norm * self.tr_final_layer(input_data)
