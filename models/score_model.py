...
from dmasif.dmasif import Surf_Extract
...
class TensorProductScoreModel(torch.nn.Module):
	def __init__(self, t_to_sigma, device, timestep_emb_func, in_lig_edge_features=4, sigma_embed_dim=32, sh_lmax=2,
				 ns=16, nv=4, num_conv_layers=2, lig_max_radius=5, surf_max_radius=5, rec_max_radius=30, cross_max_distance=250,
		...
		self.surf_max_radius = surf_max_radius                            # 5
		...
		self.confidence_mode = confidence_mode                          # F for score model
		...
		self.protein_surf_edge_embedding = nn.Sequential(nn.Linear(distance_embed_dim, ns),nn.ReLU(), nn.Dropout(dropout),nn.Linear(ns, ns))
		self.ligand_surf_edge_embedding = nn.Sequential(nn.Linear(distance_embed_dim, ns),nn.ReLU(), nn.Dropout(dropout),nn.Linear(ns, ns))
		...
		self.surf_to_ligand_cross_edge_embedding = nn.Sequential(nn.Linear(sigma_embed_dim + cross_distance_embed_dim, ns), nn.ReLU(), nn.Dropout(dropout),nn.Linear(ns, ns))
		self.surf_to_rec_cross_edge_embedding = nn.Sequential(nn.Linear(sigma_embed_dim + cross_distance_embed_dim, ns), nn.ReLU(), nn.Dropout(dropout),nn.Linear(ns, ns))
		
		self.lig_distance_expansion = GaussianSmearing(0.0, lig_max_radius, distance_embed_dim)
		self.surf_distance_expansion = GaussianSmearing(0.0, surf_max_radius, distance_embed_dim)
		...
		surf_to_rec_conv_layers, surf_to_ligand_conv_layers = [], []
		protein_surf_conv_layers, ligand_surf_conv_layers = [], []
		num_feats = [ns, ns + 3 * nv, ns + 6 * nv, 2 * ns + 6 * nv]
		for i in range(num_conv_layers):
			...
			input_c, out_c = num_feats[min(i, len(num_feats) - 1)], num_feats[min(i+1, len(num_feats) - 1)]
			...
			surf_to_ligand_conv_layer = TensorProductConvLayer(**parameters)
			surf_to_ligand_conv_layers.append(surf_to_ligand_conv_layer)
			...
			if i != num_conv_layers -1:
				...
				protein_surf_layer = TensorProductConvLayer(**parameters)
				protein_surf_conv_layers.append(protein_surf_layer)
				ligand_surf_layer = TensorProductConvLayer(**parameters)
				ligand_surf_conv_layers.append(ligand_surf_layer)

				surf_to_rec_conv_layer = TensorProductConvLayer(**parameters)
				surf_to_rec_conv_layers.append(surf_to_rec_conv_layer)
		...
		self.surf_to_rec_conv_layers = nn.ModuleList(surf_to_rec_conv_layers)
		self.surf_to_ligand_conv_layers = nn.ModuleList(surf_to_ligand_conv_layers)

		self.protein_surf_conv_layers = nn.ModuleList(protein_surf_conv_layers)
		self.ligand_surf_conv_layers = nn.ModuleList(ligand_surf_conv_layers)

		self.surf_extrace = Surf_Extract(out_channels= ns)
		self.lig_surf_extrace = Surf_Extract(out_channels= ns)
	...
	def forward(self, data):
		...
		# build cross graph for molecular and ligand
		if self.dynamic_max_cross:
			cross_cutoff = (tr_sigma * 3 + 20).unsqueeze(1)
			cross_cutoff2 = (tr_sigma + 10).unsqueeze(1)
		else:
		...
		# Build protein surface point could features
		masks_10A = self.find_protein_atom_within_10A(data, cross_cutoff2)
		surface_points, point_features, batch = self.surf_extrace(data['receptor'].pos[masks_10A], data['receptor'].batch[masks_10A])
		data['protein_surface'].pos = surface_points
		data['protein_surface'].x = point_features
		data['protein_surface'].batch_ = batch.long()

		# Build ligand surface point could features
		ligand_surface_points, ligand_point_features, ligand_batch = self.lig_surf_extrace(data['ligand'].pos, data['ligand'].batch)
		data['ligand_surface'].pos = ligand_surface_points
		data['ligand_surface'].x = ligand_point_features
		data['ligand_surface'].batch_ = ligand_batch.long()

		# build protein surface graph
		protein_surf_node_attr, protein_surf_edge_index, protein_surf_edge_attr, protein_surf_edge_sh = self.build_protein_surf_conv_graph(data)
		protein_surf_edge_attr = self.protein_surf_edge_embedding(protein_surf_edge_attr)
		protein_surf_src, protein_surf_dst = protein_surf_edge_index

		# build ligand surface graph
		ligand_surf_node_attr, ligand_surf_edge_index, ligand_surf_edge_attr, ligand_surf_edge_sh = self.build_ligand_surf_conv_graph(data)
		ligand_surf_edge_attr = self.ligand_surf_edge_embedding(ligand_surf_edge_attr)
		ligand_surf_src, ligand_surf_dst = ligand_surf_edge_index

		# build cross graph for ligand and ligand_surface
		surf_to_ligand_edge_index, surf_to_ligand_edge_attr, surf_to_ligand_edge_sh = self.build_cross_conv_graph_for_ligand_and_ligand_surface(data, cross_cutoff)
		cross_surf_to_ligand, cross_ligand_to_surf = surf_to_ligand_edge_index
		surf_to_ligand_edge_attr = self.surf_to_ligand_cross_edge_embedding(surf_to_ligand_edge_attr) # ns

		# build cross graph for protein and ligand_surface
		surf_to_protein_edge_index, surf_to_protein_edge_attr, surf_to_protein_edge_sh = self.build_cross_conv_graph_for_protein_and_protein_surface(data, cross_cutoff)
		cross_surf_to_protein, cross_protein_to_surf = surf_to_protein_edge_index
		surf_to_protein_edge_attr = self.surf_to_rec_cross_edge_embedding(surf_to_protein_edge_attr) # ns
		...
		for l in range(len(self.lig_conv_layers)):
			...
			# inter rec to ligand graph message passing
			...
			# ligand surf to ligand message passing
			surf_to_lig_edge_attr_ = torch.cat([surf_to_ligand_edge_attr, lig_node_attr[cross_surf_to_ligand, :self.ns], 
												ligand_surf_node_attr[cross_ligand_to_surf, :self.ns]], -1)   # 3ns
			lig_inter_update_by_surf = self.surf_to_ligand_conv_layers[l](ligand_surf_node_attr, surf_to_ligand_edge_index, surf_to_lig_edge_attr_, surf_to_ligand_edge_sh,
															  out_nodes=lig_node_attr.shape[0])

			if l != len(self.lig_conv_layers) - 1:
				# intra rec graph message passing
				...
				if self.debug:
					print(f'In l={l}', rec_intra_update.shape, f'NAN in rec_intra_update is {rec_intra_update.isnan().any()}')
					
				# inter ligand to rec graph message passing
				...
				if self.debug:
					print(f'In l={l}', rec_inter_update.shape, f'NAN in rec_inter_update is {rec_inter_update.isnan().any()}')
					
				# protein surf to protein message passing
				surf_to_rec_edge_attr_ = torch.cat([surf_to_protein_edge_attr, rec_node_attr[cross_surf_to_protein, :self.ns], 
										protein_surf_node_attr[cross_protein_to_surf, :self.ns]], -1)
				rec_inter_update_by_surf = self.surf_to_rec_conv_layers[l](protein_surf_node_attr, surf_to_protein_edge_index, surf_to_rec_edge_attr_,
																  surf_to_protein_edge_sh, out_nodes=rec_node_attr.shape[0])
				if self.debug:
					print(f'In l={l}', rec_inter_update_by_surf.shape, f'NAN in rec_inter_update_by_surf is {rec_inter_update_by_surf.isnan().any()}')
					
				# Update the protein point_features
				protein_surf_edge_attr_ = torch.cat([protein_surf_edge_attr, protein_surf_node_attr[protein_surf_src, :self.ns], protein_surf_node_attr[protein_surf_dst, :self.ns]], -1)
				protein_surf_intra_update = self.protein_surf_conv_layers[l](protein_surf_node_attr, protein_surf_edge_index, protein_surf_edge_attr_, protein_surf_edge_sh)
				if self.debug:
					print(f'In l={l}', protein_surf_intra_update.shape, f'NAN in protein_surf_intra_update is {protein_surf_intra_update.isnan().any()}')
					
				# Update the ligand point_features
				ligand_surf_edge_attr_ = torch.cat([ligand_surf_edge_attr, ligand_surf_node_attr[ligand_surf_src, :self.ns], ligand_surf_node_attr[ligand_surf_dst, :self.ns]], -1)
				ligand_surf_intra_update = self.ligand_surf_conv_layers[l](ligand_surf_node_attr, ligand_surf_edge_index, ligand_surf_edge_attr_, ligand_surf_edge_sh)
			...
			lig_node_attr = lig_node_attr + lig_intra_update + lig_inter_update + lig_inter_update_by_surf

			if l != len(self.lig_conv_layers) - 1:
				rec_node_attr = F.pad(rec_node_attr, (0, rec_intra_update.shape[-1] - rec_node_attr.shape[-1]))
				rec_node_attr = rec_node_attr + rec_intra_update + rec_inter_update + rec_inter_update_by_surf

				protein_surf_node_attr = F.pad(protein_surf_node_attr, (0, protein_surf_intra_update.shape[-1] - protein_surf_node_attr.shape[-1]))
				protein_surf_node_attr = protein_surf_node_attr + protein_surf_intra_update

				ligand_surf_node_attr = F.pad(ligand_surf_node_attr, (0, ligand_surf_intra_update.shape[-1] - ligand_surf_node_attr.shape[-1]))
				ligand_surf_node_attr = ligand_surf_node_attr + ligand_surf_intra_update
		...
		tr_pred = tr_pred / (tr_norm + 1e-6) * self.tr_final_layer(torch.cat([tr_norm, data.graph_sigma_emb], dim=1))
		rot_norm = torch.linalg.vector_norm(rot_pred, dim=1).unsqueeze(1)
		rot_pred = rot_pred / (rot_norm + 1e-6) * self.rot_final_layer(torch.cat([rot_norm, data.graph_sigma_emb], dim=1))
	...
	def build_protein_surf_conv_graph(self, data):
		# builds the ligand graph edges and initial node and edge features
		# compute edges
		radius_edges = radius_graph(data['protein_surface'].pos, self.surf_max_radius, data['protein_surface'].batch_)  # find edges with dis < 5
		edge_index = radius_edges
		# edge_index: 2 * (bond + new[dis < 5])
		# edge_attr: (bond + new[dis < 5]) x 4

		# compute initial features
		node_attr = data['protein_surface'].x

		src, dst = edge_index
		edge_vec = data['protein_surface'].pos[dst.long()] - data['protein_surface'].pos[src.long()]
		edge_attr = self.surf_distance_expansion(edge_vec.norm(dim=-1))
		edge_sh = o3.spherical_harmonics(self.sh_irreps, edge_vec, normalize=True, normalization='component')
		return node_attr, edge_index, edge_attr, edge_sh

	def build_ligand_surf_conv_graph(self, data):
		# builds the ligand graph edges and initial node and edge features
		# compute edges
		radius_edges = radius_graph(data['ligand_surface'].pos, self.surf_max_radius, data['ligand_surface'].batch_)  # find edges with dis < 5
		edge_index = radius_edges
		# edge_index: 2 * (bond + new[dis < 5])
		# edge_attr: (bond + new[dis < 5]) x 4

		# compute initial features
		node_attr = data['ligand_surface'].x

		src, dst = edge_index
		edge_vec = data['ligand_surface'].pos[dst.long()] - data['ligand_surface'].pos[src.long()]
		edge_attr = self.surf_distance_expansion(edge_vec.norm(dim=-1))
		edge_sh = o3.spherical_harmonics(self.sh_irreps, edge_vec, normalize=True, normalization='component')
	...
	def find_protein_atom_within_10A(self, data, cross_cutoff2):
		bs = data['receptor'].batch.max() + 1
		masks = []
		for i in range(bs):
			protein = data['receptor'].pos[data['receptor'].batch == i]
			ligand = data['ligand'].pos[data['ligand'].batch == i]
			min_dist, _ = torch.cdist(protein, ligand).min(dim=1)
			cutoff = cross_cutoff2[i].item()
			while True:
	...
	def build_cross_conv_graph_for_protein_and_protein_surface(self, data, cross_distance_cutoff_for_protein_and_surface):
		# builds the cross edges between ligand and receptor
		cross_distance_cutoff = cross_distance_cutoff_for_protein_and_surface
		if torch.is_tensor(cross_distance_cutoff):
			# different cutoff for every graph (depends on the diffusion time)
			edge_index = radius(data['protein_surface'].pos / cross_distance_cutoff[data['protein_surface'].batch_],
								data['receptor'].pos / cross_distance_cutoff[data['receptor'].batch], 1,
								data['protein_surface'].batch_, data['receptor'].batch, max_num_neighbors=10)
		else:
			edge_index = radius(data['protein_surface'].pos, data['receptor'].pos, cross_distance_cutoff,
							data['protein_surface'].batch_, data['receptor'].batch, max_num_neighbors=10)
		src, dst = edge_index
		edge_vec = data['protein_surface'].pos[dst.long()] - data['receptor'].pos[src.long()]
		edge_length_emb = self.cross_distance_expansion(edge_vec.norm(dim=-1))
	...
	def build_cross_conv_graph_for_ligand_and_ligand_surface(self, data, cross_distance_cutoff_for_protein_and_surface):
		# builds the cross edges between ligand and receptor
		cross_distance_cutoff = cross_distance_cutoff_for_protein_and_surface
		if torch.is_tensor(cross_distance_cutoff):
			# different cutoff for every graph (depends on the diffusion time)
			edge_index = radius(data['ligand_surface'].pos / cross_distance_cutoff[data['ligand_surface'].batch_],
								data['ligand'].pos / cross_distance_cutoff[data['ligand'].batch], 1,
								data['ligand_surface'].batch_, data['ligand'].batch, max_num_neighbors=10)
		else:
			edge_index = radius(data['ligand_surface'].pos, data['ligand'].pos, cross_distance_cutoff,
							data['ligand_surface'].batch_, data['ligand'].batch, max_num_neighbors=10)

		src, dst = edge_index
		edge_vec = data['ligand_surface'].pos[dst.long()] - data['ligand'].pos[src.long()]
