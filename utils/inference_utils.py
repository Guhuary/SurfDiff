...
def compute_ESM_embeddings(model, alphabet, labels, sequences):
	...
	repr_layers = [33]
	include = "per_tok"
	truncation_seq_length = 1024 # 1022
...
class InferenceDataset(Dataset):
	def __init__(self, out_dir, complex_names, protein_files, ligand_descriptions, protein_sequences, lm_embeddings,
		...
		self.receptor_radius = receptor_radius              # 15.0
		self.c_alpha_max_neighbors = c_alpha_max_neighbors  # 24
		self.remove_hs = remove_hs                      # T
		self.all_atoms = all_atoms                          # F,  T for confidence 
		...
		self.prior_pos = torch.load('ckpts/identified_pocket_center.pt')

		'''
		print('receptor_radius', self.receptor_radius)
		print('c_alpha_max_neighbors', self.c_alpha_max_neighbors)
		print('remove_hs', self.remove_hs)
		print('all_atoms', self.all_atoms)
		print('atom_radius', self.atom_radius)
		print('atom_max_neighbors', self.atom_max_neighbors)

		print('complex_names', self.complex_names)
		print('protein_files', self.protein_files)
		print('ligand_descriptions', self.ligand_descriptions)
		print('protein_sequences', self.protein_sequences)
		'''
	...
	def get(self, idx):
		...
		try:
			...
			'''
				rec: 
				rec_coords: list [num_residual, num_atom, 3]
				c_alpha_coords, n_coords, c_coords: np.array [n_residues, 3]
				lm_embeddings: [n_residues, 1280(embed dim)]
			'''
			...
			return complex_graph
		'''
			complex_graph['ligand'].x: [n_atoms, dim_atom]
			complex_graph['ligand'].pos: [n_atoms, 3]
			complex_graph['ligand', 'lig_bond', 'ligand'].edge_index: 
			complex_graph['ligand', 'lig_bond', 'ligand'].edge_attr: 
			complex_graph['ligand'].edge_mask: [edges] bool
			complex_graph['ligand'].mask_rotate: [edges, n_atoms]
			
			complex_graph['receptor'].x: [node_feat, embedding], 		n_residues x (1+D)
			complex_graph['receptor'].pos: position of each CA, 		n_residues x 3
			complex_graph['receptor'].mu_r_norm							n_residues x n_sigma
			complex_graph['receptor'].side_chain_vecs: rel position		n_residues x 2 x 3
		...
		prior = self.prior_pos[name]
		complex_graph.ligand_center = prior - protein_center
