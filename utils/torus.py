...
import math
import torch
def score_norm_torch(sigma):
    sigma = torch.log(sigma / math.pi)
    sigma = (sigma - math.log(SIGMA_MIN)) / (math.log(SIGMA_MAX) - math.log(SIGMA_MIN)) * SIGMA_N
    sigma = torch.round(torch.clip(sigma, 0, SIGMA_N)).long()
    score_norm_t = torch.tensor(score_norm_, device=sigma.device)
    return score_norm_t[sigma]