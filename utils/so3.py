...
def score_norm_torch(eps):
    eps_idx = (torch.log10(eps) - math.log10(MIN_EPS)) / (math.log10(MAX_EPS) - math.log10(MIN_EPS)) * N_EPS
    eps_idx = torch.clip(torch.round(eps_idx), 0, N_EPS-1).long()
    _exp_score_norms_t = torch.tensor(_exp_score_norms, device=eps_idx.device)
    return _exp_score_norms_t[eps_idx].float()