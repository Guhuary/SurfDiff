...
def train_epoch_rank(model, loader, optimizer, device, ema_weigths, loss_fn):
    model.train()
    ...
    for data in tqdm(loader, total=len(loader)):
        ...
        meter.add([loss.detach().cpu()])
    
    return meter.summary()

def test_epoch_rank(model, loader, device, loss_fn):
    model.eval()
    meter = AverageMeter(['loss'])
    for data in tqdm(loader, total=len(loader)):
        if device.type == 'cuda' and len(data) == 1 or device.type == 'cpu' and data.num_graphs == 1:
    ...
    for data in tqdm(loader, total=len(loader)):
        ...
        try:
            ...
            if tr_pred.isnan().any() or rot_pred.isnan().any() or tor_pred.isnan().any():
                print('Nan exists skip this batch')
            ...
            # print(tr_pred.shape, rot_pred.shape, tor_pred.shape)
