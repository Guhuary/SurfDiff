...
from utils.sampling import randomize_position, sampling_new_confidence
from utils.utils import get_model
from utils.visualise import PDBFile
from models.rank_model import Rank_Model
...
parser.add_argument('--complex_name', type=str, default='6qqw', help='Name that the complex will be saved with')
parser.add_argument('--protein_path', type=str, default='data/6qqw/6qqw_protein_processed.pdb', help='Path to the protein file')
...
parser.add_argument('--ligand_description', type=str, default='data/6qqw/6qqw_ligand.mol2', help='Either a SMILES string or the path to a molecule file that rdkit can read')
...
parser.add_argument('--model_dir', type=str, default='ckpts/score_model', help='Path to folder with trained score model and hyperparameters')
parser.add_argument('--ckpt', type=str, default='best_ema_inference_epoch_model.pt', help='Checkpoint to use for the score model')
parser.add_argument('--confidence_model_dir', type=str, default='ckpts/diffdock_confidence_model', help='Path to folder with trained confidence model and hyperparameters')
...
parser.add_argument('--rank_model_dir', type=str, default='ckpts/rank_model', help='Path to folder with trained confidence model and hyperparameters')
parser.add_argument('--rank_ckpt', type=str, default='epoch75_ema_inference_model.pt', help='Checkpoint to use for the confidence model')
...
# rank model
if args.rank_model_dir is not None:
	with open(f'{args.rank_model_dir}/model_parameters.yml') as f:
		rank_args = Namespace(**yaml.full_load(f))
...
if args.rank_model_dir is not None:
	rank_model = Rank_Model(
						num_conv_layers=rank_args.num_conv_layers,
						lig_max_radius=rank_args.max_radius,
						ns=rank_args.ns, nv=rank_args.nv,
						distance_embed_dim=rank_args.distance_embed_dim,
						cross_distance_embed_dim=rank_args.cross_distance_embed_dim,
						batch_norm=not rank_args.no_batch_norm,
						dropout=rank_args.dropout,
						use_second_order_repr=rank_args.use_second_order_repr,
						cross_max_distance=rank_args.cross_max_distance,
						lm_embedding_type='esm')
	state_dict = torch.load(f'{args.rank_model_dir}/{args.rank_ckpt}', map_location=torch.device('cpu'))
...
tr_schedule = get_t_schedule(inference_steps=args.inference_steps)  # 20
...
for idx, orig_complex_graph in tqdm(enumerate(test_loader)):
	origin_lig = orig_complex_graph['ligand'].pos.cpu().numpy()
	...
	try:
		...
		randomize_position(data_list, score_model_args.no_torsion, False, score_model_args.tr_sigma_max)    # X, F, F, 19.0
		...
		data_list, confidence, visualization = sampling_new_confidence(data_list=data_list, model=model,
										 ...
										 batch_size=args.batch_size, no_final_step_noise=args.no_final_step_noise, rank_model=rank_model)
		...
		# # save frame (My self)
		# for order in re_order:
		# 	for step in range(args.actual_steps if args.actual_steps is not None else args.inference_steps):
		# 		pos = visualization[order][step]
		# 		mol_pred = copy.deepcopy(lig)
		# 		if score_model_args.remove_hs: 
		# 			mol_pred = RemoveHs(mol_pred)
		# 		write_mol_with_coords(mol_pred, pos, os.path.join(write_dir, f'rank{order+1}_step{step}.sdf'))
