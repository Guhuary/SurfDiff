...
class NoiseTransform(BaseTransform):
	...
	def apply_noise(self, data, t_tr, t_rot, t_tor, tr_update = None, rot_update=None, torsion_updates=None):
		...
		soft_dists = [[] for _ in range(10)]
		trs = [[] for _ in range(10)]

		for num_directions in range(10):
			tr_update = torch.normal(mean=0, std=tr_sigma, size=(1, 3)) 
			print(tr_update)
			norm = np.linalg.norm(tr_update.numpy())
			...
			for i in range(n_points):
				data_i = modify_conformer(data_origin, tr_update / n_points, torch.from_numpy(rot_update).float() / n_points, torsion_updates / n_points)
				ligand = data_i['ligand'].pos
				receptor_pos = data_i['receptor'].pos
				soft_sdf = soft_distances(receptor_pos, ligand)
				soft_dists[num_directions].append(soft_sdf.item())
				trs[num_directions].append(norm / n_points * (i+1))
		soft_dists = np.array(soft_dists)
		trs = np.array(trs)
		np.savez('data.npz', soft_dists=soft_dists, trs=trs)
		assert 0==1
...
class PDBBind(Dataset):
	...
	def len(self):
		...
		# 1000 if len(self.complex_graphs) > 1000 else len(self.complex_graphs)
